...
export class TransactionService {
    ...
    async create(tenantId: string, salesPersonId: string, data: {
        ...
        try {
            ...
            this.logger.error(`Transaction Create Error: ${error.message}`, error.stack);
            if (error instanceof BadRequestException || error instanceof NotFoundException) {
                throw error;
            }
            throw new InternalServerErrorException(`Terjadi kesalahan internal: ${error.message}`);
        }
    }
